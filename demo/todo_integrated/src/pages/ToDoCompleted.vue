<template>
  <div id="todoCompleted">
    <ul class="top">
      <li v-for="(value,index) in todoList" :key="value.id" v-show="(value.isCompleted)">
...
export default {
  ...
  methods:{
    ...
    delTodo(event,todoId){
      ...
      this.todoList.splice(needDelIndex,1);
    ...
    delCheckedTodos(){
      ...
      this.todoList.forEach((value,index)=>{
        if(value.isChecked===true){
          needDelIdList.push(value.id)
        }
      })
      for(let id in needDelIdList){
        let needDelIndex=-1;
        this.todoList.forEach((value,index)=>{
          if(value.id===id){
            needDelIndex=index;
